...
$sql = 'SELECT * FROM fan_favorite_publications WHERE user_id=? AND is_public = 1 AND (publish_at IS NULL OR publish_at <= NOW()) ORDER BY sort_order DESC' . ($isAdminLte ? ' LIMIT 20' : '');
$stmt = getDB()->prepare($sql);
$stmt->execute([$artist['id']]);
$favorites = $stmt->fetchAll();

if ($isAdminLte) {
    echo renderAdminLteFanFavoriteListPage($artist, $slug, $favorites, 'publication');
    exit;
}

$pageUrl = siteUrl('/' . $slug . '/pubblicazioni-che-amo');
?>
<!doctype html>
<html lang="it">
<head>
...
<title>Pubblicazioni che ama <?= e($artist['display_name']) ?> — <?= e(siteName()) ?></title>
<meta property="og:type" content="website">
<meta property="og:title" content="Pubblicazioni che ama <?= e($artist['display_name']) ?>">
